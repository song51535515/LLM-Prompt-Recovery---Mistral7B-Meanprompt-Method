...
This project implements a mean prompt optimization solution based on Mistral 7B model for the [LLM Prompt Recovery](https://www.kaggle.com/competitions/llm-prompt-recovery/overview). The `Mistral 7B Prompt Recovery` notebook is the **original baseline**, while `Mistral7B-Meanprompt-Method` is a **modified experimental variant** that introduces a different prompt aggregation and fallback strategy.
